...
describe('creating posts', () => {
    ...
    test('without minimal parameters should succeed', async () => {
        const post = {
            title: 'Only a title',
        }
        const createdPost = await createPost(post)
...
describe('deleting a post', () => {
    test('should remove the post from the database', async () => {
        const result = deletePost(createdSamplePosts[0]._id)
        expect ((await result).deletedCount).toEqual(1)
        const deletedPost = await Post.findById(createdSamplePosts[0].id)
